...
| Google Ads Grants | NWANA Google Ads / Ad Grants account is under `admin@nwaofna.org`. Research 2026-09-22: developer tokens were sunset 2026-09-09; API access now attaches to a Google Cloud project (Test / Explorer / Basic / Standard tiers). Basic tier is required for campaign writes and needs brand verification of the Cloud project (nwaofna.org) plus an automated application. No API restrictions specific to Ad Grants accounts were found, but Ad Grants program policy must be encoded in every created campaign (search only, $2 max CPC, 5% CTR, sitelinks, conversion tracking). Service accounts are now Google's recommended backend auth: a human Ads admin adds the service-account email under Admin -> Access and security. Alternative zero-onboarding path: Google Ads scripts (paste + authorize + hourly schedule in the Ads UI), backend publishes desired campaign state as JSON. Per the July 1, 2026 ToS change, advertisers are responsible for AI-generated campaign content: campaign creation stays draft -> human review -> enable, never fully autonomous. No access has been provisioned yet. | Human steps, in the account holder's session: (1) confirm who holds Ads admin access and get the 10-digit customer ID; (2) fast path: paste/authorize/schedule the reconciliation script (working campaign in days); (3) full path: create the Cloud project, add the service account as a Standard/Admin user in the Ads account, complete brand verification, apply for Basic access. |
