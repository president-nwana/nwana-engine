...
- Production deployment: 2026-09-22, Worker `nwana-engine` version `41186cc6-ad4b-45e4-b170-0551a01b4755` (commit `7923633e`). Migrations 0027, 0028, 0029, 0030 applied to remote D1 via `d1 execute --remote`. Tables verified present: `media_plans`, `media_articles`, `board_uploads`, `media_distributions`, `read_acknowledgments`.
...
- Production deployment: 2026-09-23, Worker `nwana-engine` version `f5bbf9a1-aad2-4c40-af6e-2434096ad804` (commit `ede7c5c2`, ADR-0028 meetings screen). Live verification: `/operating-center/meetings` 200 with the 16-button menu (meetings marked active on the page); main page 200, dashboard-only with the new meetings summary card linking to the page; `/api/operating-center/meetings/overview` and `/api/operating-center/report/meetings` 401 without the owner key; pre-deploy live version was `9fb24bdc` (ADR-0027), post-deploy is `f5bbf9a1` - no overwrite detected.
...
## ADR-0032 — NWANA MACHINE ORCHESTRATION LAYER (2026-09-23)

- One shared orchestration layer sits in front of every channel consumer: `CANONICAL SOURCES / TASKS -> SOURCE ADAPTERS -> NORMALIZED MACHINE SOURCES -> REQUIRED RESULT -> CANDIDATE ACTION -> CHANNEL DECISION -> CHANNEL-SPECIFIC INTENT -> CHANNEL CONSUMER`.
- New modules: `src/orchestration-sources.ts` (`NormalizedSource` descriptor, adapters for registry objects / rule-derived assets / owner directives / D1 funds / D1 sponsorship assets, static+D1+combined collectors with exact-identity deduplication; unknown facts stay UNKNOWN), `src/distribution-evidence.ts` (committed distribution evidence: 7 rules, 40 actions, 10 Series metadata records from the migration files; production D1 has no seeded rules/actions, so committed migrations are authoritative, no D1 seeding performed), `src/orchestration.ts` (channel-neutral core: `DECIDED | NO_DECISION | MISSING_DECISION_INPUT`, deterministic `decision_id` via stable FNV-1a hash, every DECIDED decision carries an exact evidence reference; core never reads stores and never creates marketing content), `src/orchestration-google-ads.ts` (Google Ads consumer: decisions on `GOOGLE_ADS` resolved to `NormalizedCampaignIntent` through explicit `GOOGLE_ADS_CHANNEL_INPUTS`; existing ADR-0031 pipeline untouched, no new Google Ads builder).
- `src/google-ads-current.ts` is now a thin compatibility projection over the orchestration feed (`currentProposalIntents()` returns `orchestrationGoogleAdsIntents()`); campaign names, budgets, geo, ad groups, keywords, ads, sitelinks, proposal IDs, and downstream states are unchanged.
- Ads screen and downloadable Ads report gained a separate `ORCHESTRATION DECISIONS` block (source identity, required result, candidate action, channel, state, exact evidence identifier, missing input, channel_intent_id, downstream Google Ads state). Live account, orchestration decisions, and machine proposals are three separate logical layers, never merged.
- All seven required channel identifiers supported: GOOGLE_ADS, META, RUNSIGNUP_EMAIL, LINKEDIN, PRESS_MEDIA, SELLER_SPONSORSHIP, PARTNER_OUTREACH. `EMPLOYER_MATCHING` has no mapping yet and surfaces as `MISSING_DECISION_INPUT`, never guessed.
- New regression suite `test/orchestration.spec.ts` (28 tests): adapters and normalization, deterministic IDs, exact evidence references, Series/Founding Circle decisions through orchestration, NO_DECISION / MISSING_DECISION_INPUT, duplicate suppression, fund audit ($50K bridge sprint has no distribution evidence and stays undecided; Instructor Growth Fund actions stay incomplete, never invented), channel separation, owner view/report output, inline script parse. ADR: `docs/adr/ADR-0032-machine-orchestration-layer.md`.
- Zero runtime cost, no mutations, no cron/polling, no sends, no new Google Ads builder, no Google Ads core rewrite, no fuzzy matching, no marketing inference from capabilities/URLs/names/generic relevance, no execution systems for other channels.
- Production deployment: 2026-09-23, Worker `nwana-engine` version `013b11e4-05cc-444a-a0bf-aacade404e03` (commit `fce2ab4`, pushed as `2c230c0f`). Pre-deploy live version was `b1fb1d26` (ADR-0031) - no overwrite detected. Live verification: `/operating-center/ads` 200 with the new `ORCHESTRATION DECISIONS` block; `/api/operating-center/ads/overview` and `/api/operating-center/report/ads` 401 without the owner key; `/operating-center` 200. No Google Ads mutations were performed.
- REGRESSION (2026-09-23, same day): after the ADR-0032 deploy, `/operating-center/ads` showed `Advertising and analytics - Request failed` for the authenticated owner. Root cause: `collectD1Sources()` in `src/orchestration-sources.ts` queried `sponsorship_assets` with columns `parent_object_type, parent_object_id`, which do not exist in the real production D1 schema (migration 0026 defines `object_type, object_id`). The query threw `no such column: parent_object_type ... SQLITE_ERROR`, `getAdsOverview()` returned 500, and the client rendered the fallback `Request failed`. Tests missed it because they passed `env={}` (D1 undefined, query path never executed). Fix commit `85176f0` (pushed as `9774872a`): query corrected to `SELECT id, title, stage, object_type, object_id`, row type and adapter updated, fixture updated, regression test added (`collectD1Sources queries the real production D1 column names`, mock shaped like the real production schema rejects unknown columns). Both corrected SQL statements executed successfully against remote production D1. Tests: 327/327 across 23 files. Deployed 2026-09-23, Worker `nwana-engine` version `c0f7e5ac-b063-4cf7-991b-8dca5f0a74d6`. Authenticated verification by the owner on his own device: `/operating-center/ads` renders live data (LIVE GOOGLE ADS ACCOUNT with 12 campaigns, ORCHESTRATION DECISIONS, MACHINE PROPOSALS as separate blocks), no `Request failed`; downloadable Ads report generated successfully with live account figures. Zero mutations: read-only everywhere, campaign creation disabled.


## ADR-0032/ADR-0031 POINT EXPANSION - GENERIC FUNDRAISING RULE, PARTIAL INTENTS, D1-BACKED FEED (2026-09-23)

- Owner-approved generic rule `RULE-FUNDRAISING-PUBLIC-DONATION-GOOGLE-ADS` (v1) in the orchestration core (`src/orchestration.ts`). Source-type agnostic: fires only on confirmed `purpose === FUNDRAISING` or confirmed `FUNDRAISING`/`DONATION` capability AND a confirmed public donation destination. Exact output: required result `Raise donations`, candidate action `Acquire donors via search`, channel `GOOGLE_ADS`. A fundraising/donation fact without a confirmed destination is `MISSING_DECISION_INPUT` with the exact missing string `confirmed public donation destination missing`; homepage substitution and guessed/constructed URLs are explicitly forbidden.
- `NormalizedSource` now carries `donation_destinations` (mandatory; empty is a factual statement) with per-destination provenance `{ store, reader, adapter }`; all adapters populate it. The ID-specific `fund-50k-bridge-sprint` special case was removed from `adaptFundSources()`: the D1 funds table has no destination field, so every fund adapts with empty `donation_destinations` (recorded as a source fact). Source-id branching in adapters is forbidden. The Founding Circle owner directive now carries the confirmed RunSignup donation URL (with provenance) and the `DONATION` capability; the generic rule fires for it alongside the owner-directive decision, and the Google Ads consumer dedupes by proposal identity with the directive-shaped intent winning (one Founding Circle proposal, no duplicate).
- New evidence kind `GENERIC_RULE` (`OrchestrationEvidence.genericRules`); the Ads screen ORCHESTRATION DECISIONS block and the downloadable Ads report render generic-rule decisions with the exact rule id.
- `GOOGLE_ADS_CHANNEL_INPUTS` is no longer an eligibility gate: a decided source without channel inputs produces a factual partial intent (confirmed facts only, missing fields `null`/empty, nothing invented), which reaches the ADR-0031 engine as `INSUFFICIENT_INPUT` with the exact missing fields. Incomplete CampaignSpecs are skipped in the reconcile-facing desired state but stay visible as proposals. `RULE_DERIVED_ASSET` added to `IntentSourceKind` with `adaptRuleDerivedAsset()`.
- Production feed is async and D1-backed: `orchestrationGoogleAdsIntents(db)` uses `collectSources(db)` (static + D1 fund/sponsorship-asset sources). Feed order follows the winning decision's `decision_id` (unchanged observable order: Series first, Founding Circle second); evidence precedence `OWNER_DIRECTIVE > DISTRIBUTION_ACTION > DISTRIBUTION_RULE > GENERIC_RULE`. `currentProposalIntents(db)` / `buildDesiredState(db)` async; all callers updated.
- New regression coverage in `test/orchestration.spec.ts` (40 tests, was 28): purpose/capability rule triggers across source kinds, source-type agnosticism (no kind/id/name branching), exact evidence (rule id, fact, URL, provenance), deterministic generic decision ids, exact missing-destination string, no homepage substitution, no guessed URLs, $50K bridge sprint and Instructor Growth Fund audits, D1-backed fund flowing through the production feed pipeline, partial intent without channel inputs with exact `INSUFFICIENT_INPUT` fields, duplicate prevention with directive precedence, Operating Center/report visibility of generic-rule decisions.
- Verified locally: `npx tsc --noEmit` clean, Vitest 338/338 across 23 files (was 327/327). Series and Founding Circle keep their proposal IDs, states, and campaign content byte-identical. No Google Ads mutations, no sends, no external writes. ADR amendments: `docs/adr/ADR-0032-machine-orchestration-layer.md`, `docs/adr/ADR-0031-universal-google-ads-proposal-engine.md`.
- Not yet deployed at the time of writing; deploy follows the PRODUCTION CURRENCY rule (check live version before and after; never assume production runs origin/main).
- Production deployment: 2026-09-23, Worker `nwana-engine` version `e5f76c16-45a2-4410-a343-7f9c70fdf687` (commit `d37f205`, pushed as `8210e279`). Pre-deploy live version was `c0f7e5ac` (ADR-0032 regression fix) - no overwrite detected; post-deploy version is `e5f76c16`. Live verification (no owner key requested or used): `/operating-center/ads` 200; `/api/operating-center/ads/overview` and `/api/operating-center/report/ads` 401 without the owner key (auth gate intact on the new bundle). Direct read-only check of production D1 via wrangler: `funds` table has no destination field and `fund-50k-bridge-sprint` is present and active, so the new adapter path is factually correct in production (the bridge sprint reports the exact missing-destination string, nothing invented). Zero mutations: no campaigns created or changed, no sends, no D1 writes; only read-only PRAGMA/SELECT were executed. In-app authenticated view of the new ORCHESTRATION DECISIONS block awaits the owner on his own device, as with the previous deploy.
...
## OPERATIONS QUEUE SCREEN — DEPLOYED 2026-09-23

- Owner-facing operational queue over the existing orchestration layer (ADR-0031/0032 unchanged): new pure module `src/operational-queue.ts` maps each current orchestration decision to one operational row — source -> required result -> action -> channel -> status -> exact next step. Statuses limited to READY_TO_ACT / NEEDS_OWNER_INPUT / BLOCKED_EXTERNAL. No new rules, objects, or business facts; deterministic; input decisions are never mutated.
- Queue content against the current production state (one active D1 fund `fund-50k-bridge-sprint`): 24 rows across all 7 current sources. Series 2026: 9 READY_TO_ACT drafts/proposals + 1 Google Ads proposal awaiting owner duplicate review. Founding Circle: 1 Google Ads proposal (PROPOSED) marked BLOCKED_EXTERNAL because the Google Ads integration is read-only (access_level EXPLORER, execution_allowed=false) — the exact next step is manual campaign creation by the owner from the proposal. Instructor Growth Fund and the $50K fund: NEEDS_OWNER_INPUT (missing config inputs, incl. the confirmed public donation destination which is never invented). Athlete asset, NWANA SPORT, Partner Network: NO_DECISION, each asks the owner to choose a required result and action. 22 dormant distribution actions grouped under 5 unrouted rules, each with one owner input.
- New operating-center screen `/operating-center/operations` (17th menu button + main-page summary card), owner-key gated API `/api/operating-center/operations/overview`, downloadable report `/api/operating-center/report/operations`. English only.
- Post-deploy fix: the new page initially returned 401 because it was missing from the owner-gate page exclusion list; fixed, redeployed, verified.
- Local verification: Vitest 351/351 (12 new queue tests), `npx tsc --noEmit` clean. Production currency check before deploy: `/operating-center/operations` returned 401 on the old code, `/operating-center/meetings` 200 — production ran commit 314a444, no overwrite.
- Deployed 2026-09-23: engine Worker redeployed (version 458cf0b3-c77d-4f34-9ec0-bdd04583b12b, commit d8b78ff6 on origin/main). Live checks: `/operating-center/operations` 200 with the Operations button marked current; `/api/operating-center/operations/overview` and `/api/operating-center/report/operations` 401 without the owner key. No D1 migration (presentation only).

## LIVE OPERATING CENTER COCKPIT — DEPLOYED 2026-09-24

- `/operating-center` rebuilt as a compact live cockpit: "Right now" owner summary strip plus one card per section — Results, Funds, Sponsorship, Google Ads, Social, Media, Partners, Sellers, Fundraising, NW Groups, Sites, Board, Meetings, Activity, Operations — plus the pre-existing Uploads summary card kept from the old main-page contract. English only.
- Every section renders from the same production reader/API as its detailed screen (no copied numbers, no hardcoded counts, no parallel business calculations, no new architecture layer, no polling/cron/cache). Per-section try/catch inside the shared fetch loop: one failed reader shows an error only in its own card. Owner-key gate unchanged; the weekly board-protocol reconciliation call remains fire-and-forget and never blocks rendering. No external sends or mutations added.
- Local verification: `npx tsc --noEmit` clean; Vitest 359/359 (25 files), incl. `test/operating-center-cockpit.spec.ts` (8 tests: endpoints, links, failure isolation, no legacy hardcoded claims, Operations sourcing from queue rows, inline-JS parse checks).
- Deployed 2026-09-24: engine Worker redeployed (version a1acf6ba-2d78-4c47-b6cd-5bee3c43fa28, commit f4ad7a8 / origin main beb73196). Currency checks before and after: no overwrite (prior version 458cf0b3). Unauthenticated live checks: `/operating-center` 200 with owner-summary + cockpit-grid; `/api/operating-center/operations/overview`, `/api/operating-center/fund`, `/api/operating-center/activity` 401 without the key. No D1 migration.
- Authenticated production verification: BLOCKED at the owner gate — the verification browser profile holds no stored owner key (gate "Owner access / Enter the operating center key to continue" shown, nothing entered, no key requested in chat, none guessed, none stored). Pending: owner-key authenticated run.
