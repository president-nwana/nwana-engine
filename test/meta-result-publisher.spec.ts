...
describe("Meta result publisher", () => {
	...
	it("publishes Facebook and Instagram through their separate endpoints", async () => {
		...
		await expect(publishFacebookResult({ message: "Results", link: "https://example.com/results", pageToken: "secret", fetcher: facebookFetch as typeof fetch })).resolves.toEqual({ external_id: "fb-post" });

		const instagramFetch = vi.fn()
			.mockResolvedValueOnce(response({ id: "container" }))
			.mockResolvedValueOnce(response({ id: "ig-media" }));
		await expect(publishInstagramResult({ caption: "Results", imageUrl: "https://example.com/result.jpg", userToken: "secret", fetcher: instagramFetch as typeof fetch })).resolves.toEqual({ external_id: "ig-media" });
